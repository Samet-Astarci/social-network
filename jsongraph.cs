//facebook verileriyle olan analizi güncelledim, JSON çıktısını hem konsola yazdıran hem de isteğe bağlı olarak bir dosyaya (“graph.json”) kaydediyor şu an. 
...
public class SocialGraph
{
    ...
    public string ExportToJson()
    {
        ...
        {
            id = u.Key,
            size = u.Value.Friends.Count,
            color = u.Value.Friends.Count > 5 ? "red" : "blue",
            isTop5 = u.Value.Friends.Count > 5,
    ...
    static void Main(string[] args)
    {
        ...
        switch (choice)
        {
            case "1":
            {
                ...
                Console.WriteLine($"Loading from {fbPath} (weights = 1)...");
                graph.LoadFromFile(fbPath, hasWeight: false);
                break;
            }
            case "2":
            {
                Console.Write("Enter custom filename (with extension): ");
                var customFile = Console.ReadLine()?.Trim();
                string customPath = Path.Combine(Directory.GetCurrentDirectory(), customFile);
            ...
            case "3":
            {
                ...
                int uc = int.TryParse(Console.ReadLine(), out var tmpU) ? tmpU : 100;
                Console.Write("Enter edge count: ");
                int ec = int.TryParse(Console.ReadLine(), out var tmpE) ? tmpE : 200;
        ...
        // JSON çıktısını hem konsola yazdır hem dosyaya kaydet
        string json = graph.ExportToJson();
        Console.WriteLine("\n--- Graph JSON ---");
        Console.WriteLine(json);

        string outPath = Path.Combine(Directory.GetCurrentDirectory(), "graph.json");
        try
        {
            File.WriteAllText(outPath, json);
            Console.WriteLine($"\nJSON graph saved to: {outPath}");
        }
        catch (Exception ex)
        {
